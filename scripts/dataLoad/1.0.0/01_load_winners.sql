...
From event where name = 'GP Louisville'
    And Not Exists ( SELECT name FROM event_winner WHERE name = 'Tatsuumi Kobayashi' and ranking = 20 );
...
From event where name = 'GP San Jose'
    And Not Exists ( SELECT name FROM event_winner WHERE name = 'Courtney Rudigar' and ranking = 47 );
...
From event where name = 'GP Pittsburgh'
    And Not Exists ( SELECT name FROM event_winner WHERE name = 'Veronica Page' and ranking = 29 );
...
(id, version, event_id,  name, ranking, record, blurb )
    Select nextval('hibernate_sequence'),
        ...
        24,
        '12-3-0',
        'Vancouver was a nail biter in more ways than one.  The field of women was small in Canada and at the end of day one the bulk of the female competitors were sitting at 15 points, just shy of making day two.  Only 5 women were representing the Lady Planeswalkers that Sunday which is rather daunting odds. Variance being what it was I did not expect to see another Tatsuumi or Veronica this day.  But I was wrong.  Not only did a woman go 12-3 in Vancouver.  Two of them did.  Chantelle Campbell and Jennifer Crotts were both in it to win it.  They gave no ground and asked no quarter.  For the first time we had to wait for final standings to crown our winner because it came down to tiebreakers.  At 24th place Chantelle narrowly beat Jennifer''s 28th place finish. '
From event where name = 'GP Vancouver'
    And Not Exists ( SELECT name FROM event_winner WHERE name = 'Chantelle Campbell' and ranking = 24 );
